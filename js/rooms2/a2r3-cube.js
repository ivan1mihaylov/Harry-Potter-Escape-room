...
export const meta = {
  ...
  hints: [
    ...
    'Решение отначало: <b>ССИИИЮЮИ</b> за първата плоча, после <b>СЗЮЗЗЗСССС</b> за втората и <b>ЮЮИИИЮИЮЗСИССС</b> за третата. (С = север/нагоре по схемата, Ю = юг, И = изток, З = запад.)',
  ],
};
...
function renderGoals(api) {
  ...
  g.innerHTML = GOALS.map((go, i) => `
    <div class="goal ${api.data.stage > i ? 'done' : api.data.stage === i ? 'active' : ''}">
      ...
      <span>Стена <b>${go.label}</b> надолу върху ${['далечната дясна', 'близката лява', 'близката дясна'][i]} златна плоча</span>
    </div>`).join('');
}
