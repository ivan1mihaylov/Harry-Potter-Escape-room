...
export const meta = {
  ...
  title: 'Оклуменс',
  sub: 'Стълбата свършва в кръгла килия без решетка — защото не ѝ трябва. Студът идва отдолу и се изкачва по-бавно от теб, но не спира.',
  rune: null,
  grain: null,
  bg: 'dim',
  tint: '#cfd8e6',
    ...
    b.addEventListener('click', () => {
      ...
      if (o.ok) {
        ...
        if (d.probe >= PROBES.length) {
          ...
          setTimeout(() => {
            renderProbe(api);
            api.solve('Умът ти се затваря като врата в дебела стена. От другата ѝ страна остава всичко, което са ти взели — и вече знаеш пътя до него.');
          }, 1500);
        } else {
